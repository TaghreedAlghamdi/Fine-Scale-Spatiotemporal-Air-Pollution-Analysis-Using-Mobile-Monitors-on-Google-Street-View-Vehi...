...
The data used in the manuscript include both the air quality data collected by Google Street View vehicles and the land-use covariates used in Messier et. al (2018, In Review). The second data set is included in our code submission, however the first one is not. Although the air quality data are publicly available, Google requires each individual to submit an application for accessing the data separately. Therefore, we included a simulated air quality data in our submission. We plan to make both the simulated air quality data and the land-use covariates available upon publication of the manuscript.

### Availability

Restrictions (if data will not be made publicly available, justify why not)
The air quality data are publicly available. However, Google requires each individual to submit an application for accessing the data separately. Therefore, authors who wish to access the data can apply here https://docs.google.com/forms/d/e/1FAIpQLSf_4GIkK1tmVMFRSxz42KgvOM3Z3NGeOFFje_FS8FBbz1vTig/viewform .

### Description 

Permissions (demonstrate that author has legitimate access to data)
Authors have permissions from Messier et. al to access the land-use data and permission from Google to use the air quality data.
...
R code for performing the analyses in the manuscript are available on GitHub.

### Description

How delivered (R package, Shiny app, etc.)
R scripts containing the code for all analyses are made available on GitHub here: https://github.com/majohnso/JASA-mobile-AQ.
...
The code to reproduce analysis are provided, however all tables and figures presented in manuscript are based on Google data that has restricted access. Therefore, simulated data for running the analyses are included on GitHub. A description of how the data were simulated is included in a readme file on GitHub. Also provided is the link to obtain the actual data from Google.
